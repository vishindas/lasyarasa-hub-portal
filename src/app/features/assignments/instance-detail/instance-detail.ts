...
/** T10 detail + T11 late-enrollee handling. Issue/Skip (the only mutations here) are disabled during WRITE_FROZEN via late-enrollee-banner's own mutationsDisabled input. */
@Component({
  selector: 'app-instance-detail',
  standalone: true,
  imports: [DatePipe, MatButtonModule, MatIconModule, AssignmentModeBannerComponent, AssignmentMessageComponent, FullOutageBlockComponent, StatusChipAssignmentComponent, StudentRollupTableComponent, LateEnrolleeBannerComponent],
  styles: [`
  ...
  template: `
    ...
    @if (mode.mode() === 'FULL_OUTAGE') {
      ...
      @if (loading()) {
        ...
        @if (i.status === 'ACTIVE') {
          <app-late-enrollee-banner [instanceId]="i.id" [mutationsDisabled]="mode.mutationsDisabled()" (refreshed)="loadStudents()" />
        }
